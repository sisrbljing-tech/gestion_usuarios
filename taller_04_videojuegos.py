...
Descripción:
Sistema de consola para administrar el inventario y las ventas de una tienda de videojuegos.
...
# =========================================================
# FUNCIÓN MOSTRAR INVENTARIO DE VIDEOJUEGOS
# =========================================================
...
def actualizar_precio(diccionario_videojuegos):
    codigo_videojuego = input("\nIngrese código del videojuego: ").upper()
...
# =========================================================
def registrar_venta(diccionario_videojuegos,lista_historial_ventas):
    codigo_videojuego = input("\nIngrese código del videojuego: ").upper()
    if codigo_videojuego not in diccionario_videojuegos:
        print("Videojuego no encontrado.")
        return
    try:
        cantidad_vendida = int(input("Ingrese cantidad a vender: "))
        if cantidad_vendida <= 0:
            print("Cantidad inválida.")
            return
        cantidad_disponible = (diccionario_videojuegos[codigo_videojuego]["cantidad"])
        if cantidad_vendida > cantidad_disponible:
            print("Inventario insuficiente.")
            return
        precio_unitario = (diccionario_videojuegos[codigo_videojuego]["precio"])
        ...
        lista_historial_ventas.append({"codigo": codigo_videojuego,"cantidad": cantidad_vendida,"total": valor_total})
        print("\n===== FACTURA =====")
        print(f"Juego: "f"{diccionario_videojuegos[codigo_videojuego]['nombre']}")
...
def mostrar_estadisticas(diccionario_videojuegos):
    ...
    cantidad_mayor = 0
    for codigo_videojuego, datos_videojuego in diccionario_videojuegos.items():
        valor_total_inventario += (datos_videojuego["precio"] *datos_videojuego["cantidad"])
...
def eliminar_videojuego(diccionario_videojuegos):
    codigo_videojuego = input("\nIngrese código a eliminar: ").upper()
    if codigo_videojuego in diccionario_videojuegos:
...
def mostrar_inventario_bajo(diccionario_videojuegos):
    ...
    for codigo_videojuego, datos_videojuego in diccionario_videojuegos.items():
        if datos_videojuego["cantidad"] < 3:
            print(codigo_videojuego,datos_videojuego["nombre"],datos_videojuego["cantidad"])
...
def menu():
    while True:
        ...
        if opcion_usuario == "1":
            ...
            registrar_venta(diccionario_videojuegos,lista_historial_ventas)
